-- "gamemodes\\bhop\\gamemode\\essential\\timer\\cl_replaycontrols.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
-- cl_replaycontrols by justa
...
local function BuildReplayControls()
    ...
    replaycontrols.frameskip = BuildSideOption(replaycontrols, 10, 10, skipstep, 1, 100, "Set step", "frames", true, false, function(var)
        skipstep = var 
        UTIL:AddMessage("Admin", "The bot will now skip in ", tostring(var), " frame intervals.")
    end)   
    ...
    replaycontrols.CPaint = function(self, w, h)
        if not currentbot then 
            draw.SimpleText("Not spectating a bot.", "ui.mainmenu.button", w / 2, 30 + ((h - 30) / 2), color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    end 
...
vgui.Register("control_button", butt, "DButton")

NETWORK:GetNetworkMessage("botcontrols_menu", function()
    BuildReplayControls()
end)

--[[local function test(client, data)
